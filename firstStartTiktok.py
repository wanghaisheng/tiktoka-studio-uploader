...
def getCookie(browserType:str="firefox",proxyserver:str='',channelname:str='youtube-channel'):
    ...
    if browserType in ['firefox','webkit','']:
        if proxyserver:
            command="playwright codegen -b "+browserType+ " --device 'iPhone 12' --proxy-server "+proxyserver+" --lang 'en-GB' --save-storage="+channelname+"-cookie.json https://www.tiktok.com"
        else:
            command="playwright codegen -b "+browserType+"  --device 'iPhone 12' --lang 'en-GB' --save-storage="+channelname+"-cookie.json https://www.tiktok.com"       
...
if __name__ == "__main__":
    ...
    # getCookie()
    getCookie(browserType='firefox',proxyserver='socks5://127.0.0.1:1080',channelname='tiktok')