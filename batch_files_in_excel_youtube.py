...
if len(accounts)>0:
    ...
    for i in range(len(t)):
        ...
        def scheduletopublish_specific_date(videopath,thumbpath,title,description,category,publish_date,prefertags,return_dict,retry=3):
        ...
        # 每个video至多尝试upload三次
            while retry:
                ...
                try:
                    ...
                    res,videoid= asyncio.run(upload.upload(
                        videopath,
                        title=title[:95],
        ...
        for i in range(videocount):
            ...
            publish_date =datetime(today.year, today.month+monthoffset, startingday+initialday+dayoffset, initialhour, initialminute)
            ...
            if not 'thumbpath' in videofiles[i]:
                ...
                videofiles[i]['thumbpath']=videofiles[i]['videopath'].replace('.mp4','-1.jpg')
            ...
            with ThreadPoolExecutor(TASKS) as executor:
                # submit ten tasks
                futures_to_data = {executor.submit(lambda p: scheduletopublish_specific_date(*p),[videofiles[i]['videopath'],videofiles[i]['thumbpath'],videofiles[i]['title'],videofiles[i]['description'],'',videofiles[i]['publish_date'],videofiles[i]['tags'],statusdata] ) :i  for i in range(TASKS)}
                # register callbacks
