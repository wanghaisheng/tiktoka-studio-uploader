...
def db_update_repost_status(video_id):
    comments_time = time.time()
    conn = sqlite3.connect(DB_NAME) 
    
    conn.execute("insert into repost(video_id,comments_time) values('{}',{})".format(video_id,comments_time) )
    conn.commit()
  
 

def db_is_post_already_retweeted(video_id):
    conn = sqlite3.connect(DB_NAME) 
    cursor = conn.execute("SELECT * FROM repost where video_id='{}'".format(video_id))
    result = cursor.fetchone()
    if result is None:
        return False
    return True
...
def db_clear_db():
    ...
    conn.execute("DELETE FROM repost where comments_time<{}".format(one_m))
    conn.commit()
