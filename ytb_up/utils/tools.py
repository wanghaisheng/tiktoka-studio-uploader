...
import ytb_up.setting as setting
from ytb_up.db.redisdb import RedisDB
from ytb_up.utils.email_sender import EmailSender
from ytb_up.utils.log import log
...
def get_all_params(url):
    """
    >>> get_all_params("https://www.baidu.com/s?wd=ytb_up")
    {'wd': 'ytb_up'}
    """
    params_json = {}
...
def get_cache_path(filename, root_dir=None, local=False):
    ...
    if root_dir is None:
        if local:
            ...
            root_dir = os.path.join(os.path.expanduser("~"), ".ytb_up/cache")
